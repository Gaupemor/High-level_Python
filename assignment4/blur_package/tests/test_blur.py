#!/usr/bin/env python
""" 4.6: Tests for the method 'blur_image' in the module 'blur_pkg' in the package 'blur_pkg'.
...
def _set_seed():
    """ Private method that sets the random seed, so that the generated values are predictable.
    """
    np.random.seed(0)

def _generate_mock_image_file(tmp_path):
    """ Private method that arranges the image file for the unit tests testing the package.
    ...
    generated_image = np.random.uniform(low=1, high=255, size=(IMG_SIZE, IMG_SIZE, 3))
...
def _blur_mock_image_file():
    """ Private method that blurs the given image array using the package 'blur_image()' method.
